  ...
  <div ref="dropdown" class="relative inline-block text-left">
    ...
    <teleport to="body">
      <transition name="fade">
        <div v-if="isDropDown" 
          ref="menu"
          class="menu text-base-content/70 bg-base-200/80 backdrop-blur-sm border border-base-content/30 absolute rounded-box shadow-lg z-500"
          :style="menuStyle"
        >
          <!-- menu items -->
          <template v-for="(item, index) in menuItems">
            <button v-if="!item.hidden"
              :class="[
                ...
                item.disabled ? 'text-base-content/30' : 'hover:bg-base-100 hover:text-base-content hover:rounded-box cursor-pointer',
              ]"
              :key="index"
